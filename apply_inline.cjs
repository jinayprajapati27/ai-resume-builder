...
const files = ['views/index.ejs', 'views/form.ejs', 'views/layout.ejs', 'views/preview.ejs'];

for (const file of files) {
    let content = fs.readFileSync(file, 'utf8');
    
    // Check if already injected
    if (content.includes('--card: #ffffff;') && content.includes('--ring: #a78bfa;')) {
        continue; 
    }

    if (file === 'views/layout.ejs' || file === 'views/preview.ejs') {
        // Inject right before </head>
        content = content.replace('</head>', inlineStyle + '\n</head>');
    } else {
        // For index and form, inject at the beginning of the body string
